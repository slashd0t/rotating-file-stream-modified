...
describe("compression", () => {
	...
	describe("error finding external tmp file", () => {
		const events = test({ options: { compress: true, size: "10B" } }, rfs => {
			...
			rfs.findName = (attempts: any, tmp: boolean, callback: (error: Error) => void): void => {
				rfs.findName = (attempts: any, tmp: boolean, callback: (error: Error) => void): void => process.nextTick(() => callback(new Error("test")));
				prev.bind(rfs, attempts, tmp, callback)();
			};
			rfs.end("test\ntest\n");
		});
	...
	describe("error closing tmp file", () => {
		const events = test({ options: { compress: true, size: "10B" } }, rfs => {
			rfs.fsClose = (fd: number, callback: (error: Error) => void): void => {
				rfs.fsClose = (fd: number, callback: (error: Error) => void): void => process.nextTick(() => callback(new Error("test")));
				close(fd, callback);
			};
			rfs.end("test\ntest\n");
		});
